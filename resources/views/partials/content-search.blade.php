...
<article @php(post_class('mb-10 mt-5'))>
  ...
  <div x-data="{ open: false }" class="h-auto lg:hidden  py-3 flex flex-col items-center justify-center relative overflow-hidden sm:py-12">
    <div  @click="open = ! open" class=" bg-blue-100 w-full rounded flex justify-between items-stretch">
      <div class="grid grid-cols-4 gap-2 basis-full "> 
         {{ get_the_image( array('post_id' => get_the_ID(), 'scan' => true, 'image_class' => 'w-16 h-16 rounded-full col-span-1' ) ) }}
    ...
    <div x-show="open" @click.outside="open = false"  x-transition:enter="transition ease-out duration-300"
          ...
          x-transition:leave-end="opacity-0 translate-y-0" class="w-full bg-white pl-[70px] py-5 excerpt_movil  ">
